...
// Runner encapsulates model components.
type Runner struct {
	config      *llama.LlamaConfig
	weights     *llama.LlamaWeights
...
// setup loads model and tokenizer.
func setup(cfg *AppConfig) (*Runner, error) {
	fmt.Print("Loading model... ")

	config, weights, err := llama.LoadLlamaModel(cfg.CheckpointPath)
	if err != nil {
		return nil, err
	}
	...
	return &Runner{
		config:      config,
		weights:     weights,
		state:       llama.NewLlamaState(config),
...
func generate(r *Runner, cfg *AppConfig) error {
	steps := cfg.Steps
	...
	for pos < steps {
		llama.LlamaForward(token, pos, r.config, r.state, r.weights)
